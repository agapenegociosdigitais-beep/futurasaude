...
CREATE TABLE IF NOT EXISTS public.autorizacoes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  protocolo TEXT NOT NULL UNIQUE DEFAULT 'AUT-' || to_char(CURRENT_DATE, 'YYYY') || '-' || LPAD((nextval('public.autorizacoes_seq')::text), 6, '0'),
  beneficiario_id UUID NOT NULL REFERENCES public.beneficiarios(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_autorizacoes_beneficiario_id ON public.autorizacoes(beneficiario_id);
